(() => {
  ...
  document.querySelector('.desglose de animación h2').textContent = item.breakdownTitle;
  document.querySelector('.lista-de-habilidades-de-animación').innerHTML = item.skills.map((skill) => `<span>${skill}</span>`).join('');
  document.querySelector('.número-de-sección-de-animación').textContent = 'Desglose de After Effects';
  const video = document.querySelector('[data-case-video]');
  video.setAttribute('etiqueta-aria', item.media);
  video.dataset.mediaDescription = item.media;
  video.querySelector('fuente').src = `creatives/ecommerce-video/${item.source}`;
  video.load();
  video.play().catch(() => {});
